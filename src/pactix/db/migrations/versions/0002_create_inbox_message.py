...
def _upgrade_mysql() -> None:
    ...
    op.create_index('idx_inbox_message_ready', 'inbox_message', ['status', 'available_at'])
    op.create_index(
        'idx_inbox_message_fifo',
        'inbox_message',
        ['event_type', 'ordering_key', 'received_at'],
    )
    op.create_index('idx_inbox_message_lease', 'inbox_message', ['lease_until'])
    op.create_index('idx_inbox_message_correlation', 'inbox_message', ['correlation_id'])
